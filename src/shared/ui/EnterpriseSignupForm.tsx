...
export default function EnterpriseSignupForm({ onBack }: Props) {
  ...
  const handleSignup = async () => {
    if (!cguAccepted) { setError('Veuillez accepter les CGU'); return; }
    if (!form.phoneOrEmail.trim() || !form.password.trim() || !form.companyName.trim()) return;
    setLoading(true);
    setError('');
    const { error: signUpError } = await supabase.auth.signUp({
      email: form.phoneOrEmail.trim(),
