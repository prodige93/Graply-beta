...
export default function Navbar({ isMars, onAuthClick }: NavbarProps) {
        ...
        {user ? (
          <div className="hidden md:block relative">
            <button
              ...
              className="flex items-center gap-2 px-3.5 py-1.5 text-[14px] font-medium text-white/70 hover:text-white transition-colors duration-200"
            >
              <UserIcon className="w-4 h-4" />
              <span>{user.email?.split('@')[0]}</span>
            </button>
            {showUserMenu && (
              <div className="absolute top-full right-0 mt-2 w-48 rounded-lg border border-white/[0.08] bg-[#0d0d0d]/95 backdrop-blur-2xl shadow-[0_8px_32px_rgba(0,0,0,0.5)] overflow-hidden">
                <div className="p-2">
                  <button
                    onClick={handleSignOut}
                    className="w-full flex items-center gap-2 px-3 py-2 text-[14px] font-medium text-white/60 hover:text-white rounded-lg hover:bg-white/[0.04] transition-colors"
                  >
          ...
          <>
            <button
              ...
              className="hidden md:block px-3.5 py-1.5 text-[14px] font-medium text-white/70 hover:text-white transition-colors duration-200 whitespace-nowrap"
            >
              Connexion
            </button>

            <button
              onClick={() => onAuthClick(isMars ? 'enterprise' : 'creator')}
              className="hidden md:block ml-1 px-5 py-2 text-[13px] font-semibold whitespace-nowrap rounded-full transition-all duration-700"
              style={{
                color: accentColor,
        ...
        <div className="absolute top-full mt-2 left-4 right-4 md:hidden rounded-2xl border border-white/[0.08] bg-[#0d0d0d]/95 backdrop-blur-2xl shadow-[0_8px_32px_rgba(0,0,0,0.5)] overflow-hidden">
          <div className="px-4 py-3 space-y-0.5">
            ...
            <div className="pt-2 mt-2 border-t border-white/[0.06] flex flex-col gap-1">
              {user ? (
                <>
                  <div className="px-3 py-2 text-[14px] text-white/60">
                    {user.email}
                  </div>
                  <button
                    onClick={handleSignOut}
                    className="flex items-center gap-2 px-3 py-2.5 text-[14px] font-medium text-white/60 hover:text-white transition-colors"
                  >
                    <LogOut className="w-4 h-4" />
                    Déconnexion
                  </button>
                ...
                <>
                  <button
                    ...
                    className="px-3 py-2.5 text-[14px] font-medium text-white/60 hover:text-white transition-colors text-left"
                  >
                    Connexion
                  </button>
                  <button
                    onClick={() => {
                      onAuthClick(isMars ? 'enterprise' : 'creator');
                      setMobileOpen(false);
                    }}
                    className="mx-3 mb-1 py-2.5 text-center text-[13px] font-semibold rounded-full transition-all duration-700"
                    style={{
                      color: accentColor,
                      textShadow: `0 0 8px rgba(${accentRgb},0.5), 0 0 20px rgba(${accentRgb},0.25)`,
                      border: `1px solid ${accentDark}4d`,
