...
export default function ViewsChart({ series = MARQUEE_VIEWS_MONTHLY, subtitle = 'Sur la plateforme Graply' }: ViewsChartProps) {
  const uid = useId().replace(/:/g, '');
  const data = series;

  const maxVal = useMemo(() => maxChartValue(data), [data]);
  const lastValue = data[data.length - 1]!.value;
  const headline = formatCompactViews(lastValue);
  ...
  const gradArea = `viewsArea-${uid}`;
  const gradLine = `viewsLine-${uid}`;
  const filtGlow = `vGlow-${uid}`;
      ...
      <div style={{ marginBottom: 12 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
          ...
          <span style={{
            ...
            <span style={{ fontSize: 11, fontWeight: 700, color: '#FF782A' }}>+{growth}%</span>
          </span>
        </div>
      </div>
