...
import { User, Bell, MessageCircle, Search, X, Building2, CheckCircle, Home, Menu, ChevronRight, Settings, Megaphone, AlertTriangle, Users } from 'lucide-react'
...
export default function MobileNav({ onVerifyClick }: MobileNavProps = {}) {
  ...
  return (
    <>
      <header
        className={`fixed top-0 left-0 right-0 z-50 lg:hidden transition-all duration-200 ${isHome ? '' : 'hidden'}`}
        style={{
                    ...
                    {dbProfiles.map((p) => (
                      <div
                        ...
                        onClick={() => {
                          ...
                          navigate(pathTo.userProfile(p.username), { state: { from: location.pathname, backState: { searchOpen: true, searchQuery } } })
                          handleCloseSearch()
                        }}
                        className="flex items-center gap-3 px-2 py-3 rounded-xl hover:bg-white/[0.04] transition-colors cursor-pointer"
                      >
                ...
                {searchQuery.trim() && filteredCreators.map((creator) => (
                  <div
                    ...
                    onClick={() => {
                      ...
                      navigate(pathTo.createur(creator.id), { state: { from: location.pathname, backState: { searchOpen: true, searchQuery } } })
                      handleCloseSearch()
                    }}
                    className="flex items-center gap-3 px-2 py-3 rounded-xl hover:bg-white/[0.04] transition-colors cursor-pointer"
                  >
        ...
        <>
          ...
          >
            ...
            <div className="px-4 pt-5 space-y-2">
              <button
                onClick={() => { setMenuOpen(false); navigate(ROUTES.profil) }}
            ...
            <div className="mx-5 mt-auto pb-6">
              ...
              <div className="space-y-2">
                <button
                  onClick={() => { setMenuOpen(false); navigate(ROUTES.monCompte) }}
                  className="w-full flex items-center gap-3.5 px-4 py-3.5 rounded-xl transition-all duration-200 active:scale-[0.97]"
                  style={{
                    background: 'rgba(255,255,255,0.04)',
                    border: '1px solid rgba(255,255,255,0.08)',
                ...
                <button
                  onClick={() => { setMenuOpen(false); navigate(ROUTES.parametres) }}
                  className="w-full flex items-center gap-3.5 px-4 py-3.5 rounded-xl transition-all duration-200 active:scale-[0.97]"
                  style={{
                    background: 'rgba(255,255,255,0.04)',
                    border: '1px solid rgba(255,255,255,0.08)',
        ...
        <div className="flex items-center justify-around px-2 py-2">
          <button
            ...
            className={`flex flex-col items-center justify-center gap-1 px-3 py-2 rounded-xl transition-all ${
              isActive(ROUTES.home) ? 'bg-white/10' : ''
            }`}
          >
          ...
          <button
            ...
            className={`flex flex-col items-center justify-center gap-1 px-3 py-2 rounded-xl transition-all ${
              isActive(ROUTES.mesCampagnes) ? 'bg-white/10' : ''
            }`}
          >
          ...
          >
            ...
            >
              <CheckCircle className="w-5 h-5" strokeWidth={2.2} style={{ color: '#ffffff' }} />
            </div>
          </button>
          ...
          <button
            ...
            className={`flex flex-col items-center justify-center gap-1 px-3 py-2 rounded-xl transition-all ${
              isActive(ROUTES.dashboard) ? 'bg-white/10' : ''
            }`}
          >
          ...
          <button
            onClick={() => navigate(ROUTES.messagerie)}
            className={`flex flex-col items-center justify-center gap-1 px-3 py-2 rounded-xl transition-all ${
              isActive(ROUTES.messagerie) ? 'bg-white/10' : ''
            }`}
          >
