...
import { Bell, MessageCircle, User, Search, Settings, Home, X, Megaphone, CheckCircle, AlertTriangle, Users, LogOut } from 'lucide-react';
import { openVerifyModal } from '@/shared/lib/verify-event';
import { useProfile } from '@/shared/lib/use-profile';
import { useAuth } from '@/modules/auth/context/AuthContext';
const globeIcon = '/globe.svg';
const bookmarkIcon = '/bookmark_(1).svg';
const barChartIcon = '/bar_chart.svg';
const hourglassIcon = '/hourglass.svg';
...
export default function Sidebar({ activePage, onOpenSearch }: SidebarProps) {
  ...
  return (
    ...
    <aside className="hidden lg:flex w-80 shrink-0 p-6 flex-col sticky top-0 h-screen overflow-y-auto" style={{ background: 'transparent', borderRight: '1px solid rgba(255,255,255,0.05)' }}>
      <div className="space-y-6 flex-1">
        <div>
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <span onClick={() => navigate(ROUTES.profil)} className="text-sm font-semibold text-white cursor-pointer hover:text-white/80 transition-colors">@{profile?.username || 'username'}</span>
          ...
          <div className="rounded-xl p-5" style={glassCard}>
            <nav className="space-y-1.5">
              {navBtn('home', ROUTES.home, <Home className="w-5 h-5 text-white" />, 'Accueil')}
              {navBtn('mes-campagnes', ROUTES.mesCampagnes, <img src={globeIcon} alt="Globe" className="w-5 h-5" />, 'Mes campagnes')}
              {navBtn('validation-videos', ROUTES.validationVideos, <img src={hourglassIcon} alt="Verification" className="w-5 h-5" />, 'Validation')}
              {navBtn('dashboard', ROUTES.dashboard, <img src={barChartIcon} alt="Dashboard" className="w-5 h-5" />, 'Mon dashboard')}
              <button
              ...
              {navBtn('messagerie', ROUTES.messagerie, <MessageCircle className="w-5 h-5 text-white" />, 'Messagerie')}
              {navBtn('enregistre', ROUTES.enregistre, <img src={bookmarkIcon} alt="Enregistré" className="w-5 h-5" />, 'Enregistré')}
            </nav>
          </div>
        </div>
        ...
        <div className="rounded-xl p-5" style={glassCard}>
          <nav className="space-y-3">
            {accountBtn('mon-compte', ROUTES.monCompte, <User className="w-5 h-5 text-white" />, 'Mon compte')}
            {accountBtn('parametres', ROUTES.parametres, <Settings className="w-5 h-5 text-white" />, 'Paramètres')}
            <button
              onClick={() => signOut()}
              className="flex items-center gap-3 px-4 py-2.5 rounded-xl w-full text-left transition-all duration-200 hover:bg-white/[0.06] active:scale-[0.97]"
            >
