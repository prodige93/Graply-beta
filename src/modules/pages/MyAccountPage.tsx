...
const certBtnBg = '/Rectangle_658_1_(2).svg';
...
export default function MyAccountPage() {
  ...
  async function saveSocialHandle(key: SocialKey) {
    ...
    await supabase.from('profiles').update({ [col]: value, updated_at: new Date().toISOString() }).eq('id', userId);
    updateProfile({ [col]: value } as Partial<import('../lib/useProfile').Profile>);
  ...
  async function disconnectSocial(key: SocialKey) {
    const col = `${key}_handle` as const;
    await supabase.from('profiles').update({ [col]: '', updated_at: new Date().toISOString() }).eq('id', userId);
    updateProfile({ [col]: '' } as Partial<import('../lib/useProfile').Profile>);
  }
  ...
  async function saveBio() {
    setSavingBio(true);
    await supabase.from('profiles').update({ bio: newBio.trim(), updated_at: new Date().toISOString() }).eq('id', userId);
  ...
  async function saveUsername() {
    ...
    await supabase.from('profiles').update({ username: newUsername.trim(), updated_at: new Date().toISOString() }).eq('id', userId);
