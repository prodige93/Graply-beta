...
import { generateChartData, type RawChartData } from '@/shared/lib/chart-utils';
...
export default function MyCampaignDetailPage() {
  ...
  const rawPerformanceAnchors = useMemo((): RawChartData[] => {
    if (!campaign) return [];
    const tb = Math.max(totalBudget, 1);
    const factors = [0.08, 0.18, 0.32, 0.5, 0.72, 1];
    return factors.map((f, i) => ({
      label: `p${i}`,
      views: Math.round(f * tb * 55 + Math.sin(i * 1.7) * tb * 2),
      earned: Math.round(f * tb * 0.85 + Math.cos(i) * 3),
    }));
  }, [campaign, totalBudget]);

  const chartData = useMemo(
    () => (rawPerformanceAnchors.length ? generateChartData(rawPerformanceAnchors, chartPeriod) : []),
    [rawPerformanceAnchors, chartPeriod],
  );
        ...
        >
          ...
          >
            ...
            <div className="flex items-center gap-3">
              <button
                onClick={() => setConfirmDelete(false)}
                className="flex-1 px-4 py-2.5 rounded-full text-sm font-semibold text-white/60 hover:text-white transition-colors"
                style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)' }}
              >
                Annuler
              </button>
              <button
                ...
                disabled={deleting}
                className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-full text-sm font-semibold transition-all duration-200 hover:brightness-110 active:scale-[0.96] disabled:opacity-50"
